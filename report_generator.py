...
log_returns, annualized_std, mean, skewness, kurtosis, max_value, min_value = data_preparation.Data.statistics()

drawdown = round((data_preparation.Data.max_drawdown(self.returns) * 100), 4)

        calmar = round((np.exp(log_returns.mean() * 252) / abs(data_preparation.Data.max_drawdown(self.returns))), 4)

        hist_var = round((data_preparation.Data.historicalVAR(log_returns, data_preparation.Data.ALPHA)), 4)
        hist_cvar = round((data_preparation.Data.historicalCVAR(log_returns, data_preparation.Data.ALPHA)), 4)
...
def create_report(filename = 'RAPORT.pdf'):
    ...
    pdf.image(name_13, 10, 20 , max_widt h -20, 75)

    pdf.image(name_14, 10, 110 , max_widt h -20, 75)

    pdf.image(name_15, 10, 200 , max_widt h -20, 75)
